...
def scrape_events():
    ...
    # Try to use system Chrome if available (for Render.com)
    if os.getenv('CHROME_BIN'):
        options.binary_location = os.getenv('CHROME_BIN')
    
    try:
        # Try to use ChromeDriver from environment variable path or default installation
        chrome_path = os.getenv('CHROMEDRIVER_PATH', ChromeDriverManager().install())
        driver = webdriver.Chrome(service=Service(chrome_path), options=options)
    except Exception as e:
        print(f"Error setting up Chrome: {e}")
        # Fallback to basic ChromeDriver installation
